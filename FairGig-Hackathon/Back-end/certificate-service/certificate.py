...
# ==========================================
AUTH_SERVICE_URL = os.getenv("AUTH_SERVICE_URL", "https://fair-gig.onrender.com")
...
def verify_token(authorization: str = Header(None)):
    """Verify JWT token and get worker info"""
    ...
    <head>
        ...
        <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .certificate-container { max-width: 900px; margin: 0 auto; background: white; border-radius: 12px; box-shadow: 0 10px 40px rgba(0,0,0,0.1); overflow: hidden; }
            .certificate-header { background: linear-gradient(135deg, #1a56db 0%, #1e40af 100%); color: white; padding: 40px; text-align: center; }
            .certificate-header h1 { font-size: 32px; margin-bottom: 10px; }
            .certificate-body { padding: 40px; }
            .worker-info { background: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 30px; border-left: 4px solid #1a56db; }
            .worker-info h3 { color: #1e40af; margin-bottom: 15px; }
            .info-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; }
            .info-item { display: flex; justify-content: space-between; border-bottom: 1px solid #e2e8f0; padding: 8px 0; }
            ...
            .summary-card h4 { color: #64748b; font-size: 14px; margin-bottom: 10px; }
            ...
            .earnings-table th, .earnings-table td { border: 1px solid #e2e8f0; padding: 12px; text-align: left; }
            .earnings-table th { background: #f1f5f9; font-weight: 600; color: #475569; }
            .certificate-footer { background: #f8fafc; padding: 20px 40px; text-align: center; border-top: 1px solid #e2e8f0; font-size: 12px; color: #64748b; }
            .print-button { background: #1a56db; color: white; border: none; padding: 12px 30px; border-radius: 8px; cursor: pointer; font-size: 16px; margin-bottom: 20px; display: inline-block; }
        ...
        <div class="certificate-container">
            ...
            <div class="certificate-body">
                <div class="worker-info">
                    ...
                    <div class="info-grid">
                        <div class="info-item"><span class="info-label">Name:</span><span class="info-value">{{ worker_name }}</span></div>
                        <div class="info-item"><span class="info-label">Email:</span><span class="info-value">{{ worker_email }}</span></div>
                        <div class="info-item"><span class="info-label">Platform:</span><span class="info-value">{{ worker_platform }}</span></div>
                        <div class="info-item"><span class="info-label">Certificate ID:</span><span class="info-value">{{ certificate_id }}</span></div>
                        <div class="info-item"><span class="info-label">Issue Date:</span><span class="info-value">{{ issue_date }}</span></div>
                        <div class="info-item"><span class="info-label">Period:</span><span class="info-value">{{ start_date }} to {{ end_date }}</span></div>
                    </div>
                </div>
                <div class="summary-cards">
                    <div class="summary-card"><h4>Total Verified Earnings</h4><div class="value">PKR {{ total_earnings | round(2) | int }}</div></div>
                    <div class="summary-card"><h4>Total Shifts</h4><div class="value">{{ total_shifts }}</div></div>
                    <div class="summary-card"><h4>Average per Shift</h4><div class="value">PKR {{ avg_per_shift | round(2) | int }}</div><div class="sub">Lahore median: PKR {{ city_median | int }}</div></div>
                </div>
                <h3 style="margin-bottom: 15px; color: #1e40af;">Earnings Details</h3>
                <table class="earnings-table">
                    <thead><tr><th>Date</th><th>Platform</th><th>Hours</th><th>Gross (PKR)</th><th>Deductions (PKR)</th><th>Net (PKR)</th></tr></thead>
                    <tbody>
